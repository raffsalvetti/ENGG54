...
int main(int argc, char *argv[])
{
	...
	if ((sockout = socket(AF_PACKET, SOCK_RAW, IPPROTO_RAW)) == -1) {
	    perror("socket");
	}
	...
	if (ioctl(sockout, SIOCGIFINDEX, &if_idx) < 0)
	    perror("SIOCGIFINDEX");
	/* Get the MAC address of the interface to send on */
	memset(&if_mac, 0, sizeof(struct ifreq));
	strncpy(if_mac.ifr_name, ifName, IFNAMSIZ-1);
	if (ioctl(sockout, SIOCGIFHWADDR, &if_mac) < 0)
...
repeat:	printf("listener: Waiting to recvfrom...\n");
	...
	if (eh->ether_dhost[0] == DEST_MAC0 &&
		...
		if (sendto(sockout, buf, numbytes, 0, (struct sockaddr*)&socket_address, sizeof(struct sockaddr_ll)) < 0) {
	    		printf("Send failed\n");
        } else {
			printf("Package sent\n");
		}
